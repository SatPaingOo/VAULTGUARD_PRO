...
import { Activity, CpuIcon, Languages, BookOpen, ArrowLeft } from 'lucide-react';
...
export const GlobalHeader: React.FC<GlobalHeaderProps> = ({ onOpenAuth }) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full px-2 sm:px-4 md:px-8 lg:px-12 py-2 sm:py-3 md:py-6 lg:py-8 flex flex-row gap-1.5 sm:gap-2 md:gap-4 justify-between items-center z-[500] pointer-events-none bg-[#020408]/95 backdrop-blur-sm border-b border-white/5">
      ...
      <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 lg:gap-4 pointer-events-auto flex-nowrap min-w-0 flex-1 overflow-x-auto overflow-y-visible scrollbar-hide">
        ...
        <div className="glass-panel px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-4 md:py-2 lg:px-6 lg:py-3 flex items-center gap-1 sm:gap-1.5 md:gap-2 lg:gap-4 border-l-2 border-[#00d4ff] bg-black/90 shadow-2xl rounded-lg md:rounded-xl shrink-0 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-[7px] sm:text-[8px] md:text-[9px] lg:text-[11px] font-black uppercase tracking-[0.08em] sm:tracking-[0.1em] md:tracking-[0.15em] lg:tracking-[0.2em] text-white/90 whitespace-nowrap">VAULT_READY</span>
        ...
        >
          ...
          <div className="flex flex-col items-start leading-tight min-w-0">
            ...
            <span className="text-[6px] sm:text-[7px] md:text-[8px] lg:text-[9px] font-mono text-white/20 uppercase hidden sm:block">SYS_CORE</span>
          </div>
        </div>
        ...
        {!isAcademyPage && (
          ...
          >
            ...
            <span className="text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-[#00d4ff] whitespace-nowrap">
              <span className="inline sm:hidden">ACADEMY</span>
              <span className="hidden sm:inline md:hidden">ACADEMY</span>
              <span className="hidden md:inline">VAULT_ACADEMY</span>
            </span>
          </button>
        )}
        ...
        {isAcademyPage && (
          ...
          >
            ...
            <span className="text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-[#00d4ff] whitespace-nowrap">
              ...
              <span className="hidden md:inline">← BACK_TO_SCANNER</span>
            </span>
          </button>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 pointer-events-auto justify-end shrink-0 flex-shrink-0 ml-1.5 sm:ml-2 md:ml-4">
        <div className="glass-panel px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-4 md:py-2 lg:px-5 lg:py-3 rounded-lg md:rounded-xl border border-white/10 bg-black/90 flex items-center gap-1 sm:gap-1.5 md:gap-2 lg:gap-3 shadow-2xl shrink-0 flex-shrink-0 whitespace-nowrap">
          <Languages size={11} className="text-[#00d4ff] shrink-0 sm:w-[12px] sm:h-[12px] md:w-[14px] md:h-[14px] lg:w-[18px] lg:h-[18px]" />
          <select
            value={language}
            onChange={(e) => setLanguage(e.target.value as 'en' | 'mm')}
            className="bg-transparent text-[8px] sm:text-[9px] md:text-[10px] lg:text-[11px] font-black uppercase text-white outline-none cursor-pointer appearance-none pr-3 sm:pr-4 min-w-[45px] sm:min-w-[50px] whitespace-nowrap"
          >
