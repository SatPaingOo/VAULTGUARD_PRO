...
# VaultGuard Pro: Neural Mission Blueprints (v1.6.2)
...
**v1.4.2 – Probe filter, Trust UI, Console logs in dev only**: **Probe filter**: `hooks/useScanner.ts` – `filterProbesByTargetDomain()`; only probes under target domain executed; localhost/non-target skipped; report `activeProbes` filtered to target domain. **AI prompt**: `services/geminiService.ts` – activeProbes must be under target domain only; TECH_FINGERPRINT instructs AI to MUST add JavaScript/TypeScript and npm or Node.js when frontend in fingerprint; no React/Vue/Svelte without evidence. **Vercel hostname**: `hooks/useScanner.ts` – when target host is `*.vercel.app`, Vercel and Node.js added to techFingerprint. **SSL limitation**: Message updated to "SSL grade unavailable (SSL Labs API not accessible from browser)". **Trust UI**: Results DATA_QUALITY – when trust &lt; 80%, limitations section title shows "Why is trust low?" (locale `results.why_trust_low`). **Console logs**: addLog in useScanner prints [VG] to console only when `import.meta.env.DEV` or `MODE === 'development'`; no logs in production deploy.
